...
const getLayouts = () => {
    ...
    const minRecentTradesHeight = 21;
    ...
    const openOrdersHeight = 21;

    const orderBookHeight = gridHeight >= minGridHeight ?
        pixelsToGridUnits(gridHeight - margin - gridUnitsToPixels(orderHeight, rowHeight, margin), rowHeight, margin) + 3.5 :
        minOrderBookHeight;

    const recentTradesHeight = orderBookHeight;
    const tradingChartHeight = orderBookHeight;

    return {
        lg: [
            { x: 16, y: 60, w: 8, h: orderHeight, i: '1', minH: orderHeight, maxH: orderHeight, minW: 4, isDraggable: isDraggable, isResizable: isResizable },
            ...
            { x: 20, y: 0, w: 4, h: recentTradesHeight, i: '5', minH: minRecentTradesHeight, minW: 4, isDraggable: isDraggable, isResizable: isResizable },
        ],
        md: [
